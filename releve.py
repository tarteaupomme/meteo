...
def db():
    """enregistrement dans la base de donnees sqlite3"""
    ...
    cursor.execute("""CREATE TABLE IF NOT EXISTS meteo(year INT, month INT,
     day INT, hour INT, minute INT, second INT, temp REAL, pressure REAL)""")
    ...
    while True:
        ...
        data = (now.year, now.month, now.day, now.hour, now.minute, now.second,
                ...
                bmp.read_sealevel_pressure(altitude_m=232.0).__round__(0))
...
def txt():
    """enregistrement dans un simple fichier texte"""
    bmp = BMP085.BMP085(mode=BMP085.BMP085_ULTRAHIGHRES)
